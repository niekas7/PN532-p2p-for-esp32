...
bool PN532::inJumpForDEP(uint8_t baudrate, bool passive)
{
    // Command: InJumpForDEP (0x56)
    // Passive mode (1) at 106 kbps by default, using default initiator data (no GB).
    pn532_packetbuffer[0] = PN532_COMMAND_INJUMPFORDEP;
    pn532_packetbuffer[1] = passive ? 0x01 : 0x00; // 1=passive, 0=active
    pn532_packetbuffer[2] = baudrate;              // 0x00=106k, 0x01=212k, 0x02=424k
    pn532_packetbuffer[3] = 0x00;                  // use default initiator data
    ...
    // First byte is status (0x00 = OK)
    if (pn532_packetbuffer[0] != 0x00) {
        return false;
    }
    // Target number usually returned at byte 1; default to 1 if missing
    inListedTag = (status > 1) ? pn532_packetbuffer[1] : 1;
    return true;
}
